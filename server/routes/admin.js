...
router.get('/points-of-interest', async (req, res) => {
    ...
    if (!skiAreaIdString) {
        return res.status(400).json({ error: "A 'ski_area_id' query parameter is required." });
    }

    const ski_area_id = parseInt(skiAreaIdString, 10);

    if (isNaN(ski_area_id)) {
        return res.status(400).json({ error: "The 'ski_area_id' must be a valid number." });
    }
...
router.post('/points-of-interest', async (req, res) => {
    const { ski_area_id, name, type, latitude, longitude, aliases } = req.body;
...
router.post('/lifts', async (req, res) => {
    const { ski_area_id, name, start_point_id, end_point_id, lift_type, estimated_time_minutes } = req.body;
    const { data, error } = await supabase
        .from('lifts')
        .insert([{ ski_area_id, name, start_point_id, end_point_id, lift_type, estimated_time_minutes }])
        .select();
    if (error) return res.status(500).json({ error: error.message });
    res.status(201).json(data[0]);
});
...
router.put('/lifts/:id', async (req, res) => {
    const { id } = req.params;
    const { name, start_point_id, end_point_id, lift_type, estimated_time_minutes } = req.body;
    const { data, error } = await supabase
        .from('lifts')
        .update({ name, start_point_id, end_point_id, lift_type, estimated_time_minutes })
        .eq('id', id)
        .select();
    if (error) return res.status(500).json({ error: error.message });
    res.json(data[0]);
...
router.post('/trails', async (req, res) => {
    const { ski_area_id, name, difficulty, is_groomer, has_moguls, is_trees, is_steep, is_official_run } = req.body;
    const { data, error } = await supabase
        .from('trails')
        .insert([{ ski_area_id, name, difficulty, is_groomer, has_moguls, is_trees, is_steep, is_official_run }])
        .select();
    if (error) return res.status(500).json({ error: error.message });
    res.status(201).json(data[0]);
});
...
router.put('/trails/:id', async (req, res) => {
    const { id } = req.params;
    const { name, difficulty, is_groomer, has_moguls, is_trees, is_steep, is_official_run } = req.body;
    const { data, error } = await supabase
        .from('trails')
        .update({ name, difficulty, is_groomer, has_moguls, is_trees, is_steep, is_official_run })
        .eq('id', id)
        .select();
    if (error) return res.status(500).json({ error: error.message });
    res.json(data[0]);
...
router.post('/trail-segments', async (req, res) => {
    const { trail_id, start_point_id, end_point_id, estimated_time_minutes, requires_hike } = req.body;
    
    const { data, error } = await supabase
        .from('trail_segments')
        .insert([{ trail_id, start_point_id, end_point_id, estimated_time_minutes, requires_hike }])
        .select()
        .single();
...
router.put('/trail-segments/:id', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('trail_segments')
        .update({ trail_id, start_point_id, end_point_id, estimated_time_minutes, requires_hike })
        .eq('id', id)
        .select()
        .single();
