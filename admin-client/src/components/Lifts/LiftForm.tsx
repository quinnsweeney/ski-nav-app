...
export default function LiftForm({ resortId, pois, onSave, onCancel, editingLift }: LiftFormProps){
    const [name, setName] = useState(editingLift?.name || '');
    const [liftType, setLiftType] = useState(editingLift?.lift_type || 'high-speed-quad');
    const [startPointId, setStartPointId] = useState<number | null>(editingLift?.start_point_id || null);
    const [endPointId, setEndPointId] = useState<number | null>(editingLift?.end_point_id || null);
    const [estimatedTimeMinutes, setEstimatedTimeMinutes] = useState<number | null>(editingLift?.estimated_time_minutes || null);
        ...
        <Sheet variant="outlined" sx={{ p: 2, borderRadius: 'sm', my: 2 }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <FormControl>
                    ...
                    <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Panorama Express" />
                </FormControl>
                <FormControl>
                ...
                <FormControl>
                    ...
                    <Input 
                        ...
                        onChange={e => setEstimatedTimeMinutes(e.target.value ? parseInt(e.target.value) : null)} 
                        placeholder="e.g., 5"
                    />
                </FormControl>
                <FormControl>
                    <FormLabel>Lift Type</FormLabel>
                    <Input value={liftType} onChange={e => setLiftType(e.target.value)} placeholder="e.g., high-speed-quad" />
                </FormControl>
                <Box sx={{ display: 'flex', gap: 1 }}>
                    <Button type="submit" loading={isSubmitting}>Save Lift</Button>
