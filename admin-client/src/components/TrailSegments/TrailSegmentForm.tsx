...
import { Autocomplete, Box, Button, Checkbox, FormControl, FormLabel, Input, Option, Select, Sheet, Typography } from "@mui/joy";
...
export default function TrailSegmentForm({ pois, trails, onSave, onCancel, editingSegment }: TrailSegmentFormProps) {
    const [trailId, setTrailId] = useState<number | null>(editingSegment?.trail_id || null);
    const [startPointId, setStartPointId] = useState<number | null>(editingSegment?.start_point_id || null);
    const [endPointId, setEndPointId] = useState<number | null>(editingSegment?.end_point_id || null);
    const [estimatedTime, setEstimatedTime] = useState(editingSegment?.estimated_time_minutes?.toString() || '');
    const [requiresHike, setRequiresHike] = useState(editingSegment?.requires_hike || false);
        ...
        <Sheet variant="outlined" sx={{ p: 2, borderRadius: 'sm', my: 2 }}>
            ...
            <form onSubmit={(e) => e.preventDefault()} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <FormControl>
                    ...
                    <Input type="number" value={estimatedTime} onChange={e => setEstimatedTime(e.target.value)} placeholder="e.g., 5" />
                </FormControl>
                <FormControl>
                    <Checkbox
                        label="Requires Hike"
                ...
                <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    {!editingSegment && (
                        <Button onClick={() => handleSubmit(false)} loading={isSubmitting} color="primary">
                            Save & Add Next
                        </Button>
                    )}
                    <Button onClick={() => handleSubmit(true)} loading={isSubmitting} variant="soft" color="success">
