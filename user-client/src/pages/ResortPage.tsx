...
export default function ResortPage() {
  ...
  const initialFormValues = useMemo(() => {
    ...
    return {
      ...
      maxDifficulty:
        typeof params.difficulty === "string" ? params.difficulty : "blue",
      avoidLifts:
        typeof params.avoid === "string"
          ? params.avoid.split(",").map(Number)
          : [],
    };
  }, [location.search]);
  ...
  useEffect(() => {
    api
      .get<Resort[]>("/resorts")
      .then((res) => {
        const foundResort = res.data.find(
          (r) => slugify(r.name) === resortSlug
        );
        if (foundResort) {
  ...
  useEffect(() => {
    if (
      resort &&
      initialFormValues.startPointId &&
  ...
  return (
    <>
      ...
      {showAlert && (
        ...
        >
          <div>
            <Typography level="title-lg">Notice</Typography>
            <Typography level="body-sm">
              Not all trails, POIs, and lifts are in the database yet, as it is
              a manual process. If you want a route that is guaranteed to work,
              start or end your route at Eagle Wind and the Mary Jane Lodge.
              Plenty of other routes work too, but not all that should.
            </Typography>
          </div>
        </Alert>
      )}
